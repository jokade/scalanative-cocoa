...
//  @inline def firstObject(): Ptr[Byte] = extern
//  @inline def lastObject(): Ptr[Byte] = extern
...
//  @inline def arrayWithObject_[T<:NSObject](anObject: T): NSArray[T] = extern
  @inline def arrayWithObjects_count_[T<:NSObject](objects: Ptr[id], cnt: NSUInteger): id = extern
...
//  // TODO: use Iterable instead of Seq
  def arrayWithObjects[T<:NSObject](objects: Seq[T]): NSArray[T] = Zone { implicit z =>
    ...
    for(i<-0 until count)
      !(array + i) = objects(i).__ptr
    new NSArray(arrayWithObjects_count_(array,count.toULong))
//    runtime.objc_msgSend(__cls,__sel_arrayWithObjects_cnt_,array,count).cast[NSArray[T]]
